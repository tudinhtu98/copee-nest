...
export class MediaService {
  ...
  private root(): string {
    return resolve(this.config.get<string>('MEDIA_DIR') || join(process.cwd(), 'storage', 'media'));
  }

  async store(
    userId: string,
    input: Buffer,
    meta: { source: MediaSource; prompt?: string; model?: string; productId?: string },
  ...
  async findMany(userId: string, ids: string[]): Promise<MediaAsset[]> {
    ...
    const rows = await this.prisma.mediaAsset.findMany({ where: { userId, id: { in: ids } } });
  ...
  async remove(userId: string, id: string): Promise<void> {
    ...
    const inUse = await this.prisma.contentPost.count({
      where: { userId, mediaIds: { has: id }, status: { in: ['DRAFT', 'FAILED', 'PUBLISHING'] } },
    });
    if (inUse) {
      throw new ConflictException('Ảnh đang được dùng trong bài viết chưa đăng. Gỡ khỏi bài trước khi xoá.');
    }
  ...
  async importFromUrl(userId: string, url: string, productId?: string): Promise<MediaAssetDto> {
    if (!/^https?:\/\//i.test(url)) throw new BadRequestException('Link ảnh không hợp lệ');
    let res: Response;
    try {
    ...
    if (!res.ok) throw new BadRequestException(`Không tải được ảnh (HTTP ${res.status})`);
    const buffer = Buffer.from(await res.arrayBuffer());
    if (buffer.length > MEDIA_MAX_UPLOAD_MB * 1024 * 1024) throw new BadRequestException('Ảnh quá lớn');
    return this.store(userId, buffer, { source: 'PRODUCT', ...(productId ? { productId } : {}) });
  }
}
 ...
 */
export async function normalizeImage(input: Buffer): Promise<{ data: Buffer; width: number; height: number }> {
  ...
  try {
    for (const quality of [88, 75, 60]) {
      ...
      if (data.length <= FACEBOOK_MAX_BYTES) return { data, width: info.width, height: info.height };
    }
  } catch {
    throw new BadRequestException('File không phải ảnh hợp lệ hoặc định dạng chưa hỗ trợ (dùng JPG, PNG hoặc WebP).');
  }
  throw new BadRequestException('Ảnh quá lớn, không nén được xuống dưới 4MB theo giới hạn của Facebook.');
}
